									...
									<form action="{{ route('updatecar',$cars->id) }}" enctype="multipart/form-data" method="post" id="demo-form2" data-parsley-validate class="form-horizontal form-label-left">
										...
										<div class="item form-group">
											...
											<div class="col-md-6 col-sm-6 ">
												<input type="text" id="title" required="required" class="form-control " name="tite" value="{{$cars->title}}">
											</div>
											@error('title')
												{{ $message }}
											@enderror
										...
										<div class="item form-group">
											...
											<div class="col-md-6 col-sm-6 ">
												<textarea id="content" name="description" required="required" class="form-control">{{$cars->description}}</textarea>
											</div>
											@error('description')
												{{ $message }}
											@enderror
										...
										<div class="item form-group">
											...
											<div class="col-md-6 col-sm-6 ">
												...
												<img src="{{ asset('assets/images/' . $cars->image ) }}" alt="" style="width: 300px;">
											</div>
											@error('image')
												{{ $message }}
											@enderror
											<input type="hidden" name="oldImageName" value="{{ $cars->image }}">
										</div>
											...
											<div class="col-md-6 col-sm-6 ">
												<select class="form-control" name="category_id" id="">
													...
													@foreach($categories as $category) 
														<option value="{{ $category->id }}" {{ $category->id == $cars->category_id ? 'selected' : '' }}>
														{{ $category->cat_name }}
														</option>        
           											@endforeach 
												</select>
